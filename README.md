...
- **Upload and analyze one or two songs at once** — the app shows two upload slots (Song A / Song B), each independent.
...
- **SVG fingerprint visualization** — each song's RMS and spectral centroid are drawn as two overlaid lines. When two songs are loaded, both are scaled against a *shared* maximum computed across both songs, so their relative loudness/brightness is visually comparable rather than each song being normalized against only its own peak.
...
- **Basic error handling** — uploading a non-audio or corrupt file returns a clear error message (both from the backend and shown in the UI) instead of a raw stack trace or a stuck "Analyzing..." state.
...
- **No automated frontend component tests.** The frontend test suite covers the shared-scaling calculation only (a pure function); there's no automated testing of the upload flow, rendering, or playback behavior in a browser.
